...
- 夸克 / 115 链接去重：
  ...
  - 当前批次内部去重，
  - 与目标频道历史链接比对去重，
  - 同一条消息中夸克链接优先于 115 链接作为去重依据，
  - 115 链接可在“转发设置”中单独勾选开启；开启后支持 `https://115cdn.com/s/<token>` 与 `https://hdhive.com/resource/115/<token>`，`115cdn.com` 会忽略密码参数和访问码片段，并支持正文、蓝字超链接与按钮 URL。
- 锁文件防并发运行。
- 媒体下载 -> 发送 -> 临时文件清理。
...
- RSS 订阅：生成带 token 的 RSS 地址，支持开关与条数配置；有缓存时立即返回并后台刷新，条目全文、明文链接、蓝字超链接与 450x450 内的等比优化主图会输出为可点击/可展示内容。
- 自动运行：支持后台定时自动触发。
